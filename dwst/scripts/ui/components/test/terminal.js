...
describe('terminal module', () => {
  describe('createLines function', () => {
    it('should break into lines', () => {
      expect(createLines(['foo'])).to.deep.equal([['foo']]);
      ...
      expect(
        createLines(['foo', { text: '\\n' }, { text: '\\n' }, 'bar']),
      ).to.deep.equal([['foo', { text: '\\n' }], [{ text: '\\n' }], ['bar']]);
      expect(
        createLines(['foo', { text: '\\r' }, { text: '\\n' }, 'bar']),
      ).to.deep.equal([['foo', { text: '\\r' }, { text: '\\n' }], ['bar']]);
      expect(
        createLines(['foo', { text: '\\n' }, { text: '\\r' }, 'bar']),
      ).to.deep.equal([['foo', { text: '\\n' }, { text: '\\r' }], ['bar']]);
      expect(
        createLines(['foo', { text: '\\r' }, { text: '\\r' }, 'bar']),
      ).to.deep.equal([['foo', { text: '\\r' }], [{ text: '\\r' }], ['bar']]);
